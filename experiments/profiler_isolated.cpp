// profiler_isolated.cpp — Per-method isolated profiling for xctrace
// Usage: ./profiler_isolated <method> <dataset_file>
// method: spps|louds|fb|pb
// Runs a SINGLE method on a single dataset, 5 times, for clean xctrace
// profiling
...
void runFB(int n, const vector<vector<int>> &tree) {
  ...
  struct Frame {
    int id;
    int ci;
    vector<flatbuffers::Offset<FB_Bench::Node>> co;
  };
  stack<Frame> stk;
  stk.push({1, 0, {}});
  ...
  while (!stk.empty()) {
    ...
    if (f.ci < (int)tree[f.id].size()) {
      int child = tree[f.id][f.ci];
      f.ci++;
      stk.push({child, 0, {}});
    } else {
      auto off =
          FB_Bench::CreateNode(builder, f.id, builder.CreateVector(f.co));
      stk.pop();
      if (!stk.empty())
        stk.top().co.push_back(off);
      else
        result = off;
    }
  }
  ...
  while (!stk2.empty()) {
    ...
    if (f.n->children() && f.idx < (int)f.n->children()->size()) {
      stk2.push({f.n->children()->Get(f.idx), 0});
      f.idx++;
    } else
      stk2.pop();
  }
...
void runPB(int n, const vector<vector<int>> &tree) {
  ...
  PB_Bench::Node *pbRoot =
      google::protobuf::Arena::Create<PB_Bench::Node>(&encode_arena);
  pbRoot->set_id(1);
  struct Frame {
    int id;
    int ci;
  ...
  PB_Bench::Node *decRoot =
      google::protobuf::Arena::Create<PB_Bench::Node>(&decode_arena);
  decRoot->ParseFromString(data);
  volatile int sink = 0;
  struct PBF {
    const PB_Bench::Node *n;
    int idx;
  };
  stack<PBF> stk2;
  stk2.push({decRoot, 0});
  while (!stk2.empty()) {
    ...
    if (f.idx < f.n->children_size()) {
      stk2.push({&f.n->children(f.idx), 0});
      f.idx++;
    } else
      stk2.pop();
  }
...
int main(int argc, char *argv[]) {
  ...
  if (n == 0) {
    cerr << "Failed to load " << dsfile << endl;
    return 1;
  }

  cout << "Method: " << method << ", Dataset: " << dsfile << ", n=" << n
       << endl;

  // Warmup
  if (method == "spps")
    runSPPS(n, tree);
  else if (method == "louds")
    runLOUDS(n, tree);
  else if (method == "fb")
    runFB(n, tree);
  else if (method == "pb")
  ...
  // Capture counters BEFORE the 5-iteration loop
  ...
  // Capture counters AFTER the 5-iteration loop
  proc_pid_rusage(getpid(), RUSAGE_INFO_V4, (rusage_info_t *)&ru_end);

  uint64_t isolated_instructions = ru_end.ri_instructions - ru_start.ri_instructions;
  uint64_t isolated_cycles = ru_end.ri_cycles - ru_start.ri_cycles;

  cout << "--- Isolated PMC (5 iterations only) ---" << endl;
  cout << "  Instructions: " << isolated_instructions << endl;
  cout << "  Cycles:       " << isolated_cycles << endl;
  cout << "  IPC:          " << (double)isolated_instructions / isolated_cycles << endl;
  cout << "  Instr/node (per iter): " << (double)isolated_instructions / (5.0 * n) << endl;
  cout << "  Cycles/node (per iter): " << (double)isolated_cycles / (5.0 * n) << endl;
