// profiler.cpp — Profiling binary for xctrace CPU Counters
// Runs SPPS, LOUDS, FlatBuffers, Protobuf sequentially
// Each method separated by a sleep() marker for instrument segmentation
...
// =========================================================================
int main() {
    cout << "========== XCTRACE CPU COUNTERS PROFILING RUN ==========" << endl;

    struct DS { string name; string file; int n; };
    vector<DS> datasets;
    { int n = 0; loadEdgeList("real_ast_benchmark.txt", n);
      if (n > 0) datasets.push_back({"Django AST", "real_ast_benchmark.txt", n}); }
    { int n = 0; loadEdgeList("sqlite3_ast_edges.txt", n);
      if (n > 0) datasets.push_back({"sqlite3 AST", "sqlite3_ast_edges.txt", n}); }
    { int n = 0; loadEdgeList("xmark_edges.txt", n);
      if (n > 0) datasets.push_back({"XMark XML", "xmark_edges.txt", n}); }
