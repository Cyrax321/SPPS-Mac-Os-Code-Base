...
int main() {
    ...
    {
        int n = 2325575;
        auto tree = loadEdgeList("real_ast_benchmark.txt", n);
        if (n > 0) datasets.push_back({"Django AST (C1)", "real_ast_benchmark.txt", n});
    }
    // C2: sqlite3 AST
    {
        int n = 0;
        auto tree = loadEdgeList("sqlite3_ast_edges.txt", n);
        if (n > 0) datasets.push_back({"sqlite3 AST (C2)", "sqlite3_ast_edges.txt", n});
    }
    // C3: XMark XML
    {
        int n = 0;
        auto tree = loadEdgeList("xmark_edges.txt", n);
        if (n > 0) datasets.push_back({"XMark XML (C3)", "xmark_edges.txt", n});
    }
